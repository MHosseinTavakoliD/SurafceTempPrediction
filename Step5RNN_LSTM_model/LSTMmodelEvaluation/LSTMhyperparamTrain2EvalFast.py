...
for model_type in model_types:
    best_hyperparams = {}
    # Step 1: Optimize the number of units
    print(f"Training model:  {model_type}")
    best_val_mse = float('inf')
    for units in units_options:
        print (f"Testing units {units} ")
        model = define_model(model_type, sample_input_shape, units, 0.2)  # Average dropout
        ...
        history = model.fit(X_train, Y_train, epochs=epoch, batch_size=32, validation_data=(X_val, Y_val), verbose=1)  # Average batch size
        min_val_mse = min(history.history['val_mse'])
        if min_val_mse < best_val_mse:
            best_val_mse = min_val_mse
            best_hyperparams['units'] = units

    # Step 2: Optimize the dropout rate
    best_val_mse = float('inf')
    for dropout_rate in dropout_options:
        print(f"Testing dropout_rate {dropout_rate} ")
        model = define_model(model_type, sample_input_shape, best_hyperparams['units'], dropout_rate)
        optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
        model.compile(optimizer=optimizer, loss='mean_squared_error', metrics=['mse', 'mae'])
        history = model.fit(X_train, Y_train, epochs=epoch, batch_size=16, validation_data=(X_val, Y_val), verbose=1)
        min_val_mse = min(history.history['val_mse'])
        if min_val_mse < best_val_mse:
            best_val_mse = min_val_mse
            best_hyperparams['dropout_rate'] = dropout_rate

    # Step 3: Optimize the batch size
    best_val_mse = float('inf')
    for batch_size in batch_sizes:
        print(f"Testing batch_size {batch_size} ")

        model = define_model(model_type, sample_input_shape, best_hyperparams['units'], best_hyperparams['dropout_rate'])
        optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
        model.compile(optimizer=optimizer, loss='mean_squared_error', metrics=['mse', 'mae'])
    ...
    # Step 4: Optimize the learning rate
    best_val_mse = float('inf')
    for lr in learning_rates:
        print(f"Testing learning_rates {lr} ")

        model = define_model(model_type, sample_input_shape, best_hyperparams['units'], best_hyperparams['dropout_rate'])
        optimizer = tf.keras.optimizers.Adam(learning_rate=lr)
        model.compile(optimizer=optimizer, loss='mean_squared_error', metrics=['mse', 'mae'])
        history = model.fit(X_train, Y_train, epochs=epoch, batch_size=best_hyperparams['batch_size'], validation_data=(X_val, Y_val), verbose=1)
        min_val_mse = min(history.history['val_mse'])
        if min_val_mse < best_val_mse:
            best_val_mse = min_val_mse
            best_hyperparams['learning_rate'] = lr

        best_hyperparams_per_model[model_type] = best_hyperparams
# Print out the best hyperparameters
print("Best Hyperparameters per Model:")
for model_type, params in best_hyperparams_per_model.items():
