...
# Create dataset
X, Y, station_names = create_dataset(df, look_back=24, forecast_horizon=12)
...
model.add(Dense(12))  # Assuming your output size is 12
...
# Assumptions
look_back = 24  # Number of time steps you're looking back
...
X_val_reshaped = X_val_trimmed.reshape(X_val.shape[0], look_back, num_features)
...
for i in range(len(list_index)):
    ...
    # Extract past surface temperatures for the look_back period
    # Reshape X_val to 2D for easier indexing
    print (X_val_reshaped[random_index])
    past_surface_temps = X_val_reshaped[random_index, :, feature_index_for_surface_temp ]  # Extracting past surface temps
    print ("past_surface_temps", past_surface_temps)

    plt.figure(figsize=(15, 6))

    # Plot past surface temperature
    plt.plot(range(-look_back, 0), past_surface_temps, label='Past Surface Temp', color='green')
